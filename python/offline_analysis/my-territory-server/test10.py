...
def main():
    in_path = "F:\\项目文档\\绛门\\陆领项目\\0315\\广州沈海高速(初采)-1-纠偏_剪切_mvd.csv"
    #in_path = "F:\\项目文档\\绛门\\陆领项目\\0315\\original_tracks.csv"
    out_path = "F:\\项目文档\\绛门\\陆领项目\\0315\\data\\0次变道\\144\\"
    ...
    for index, row in pd.iterrows():
        ...
        x_arr.append(fractional_center_x * 3840 / 9.33)
        y_arr.append(fractional_center_y * 2160 / 9.33)
    ...
    calc_x_arr, calc_x_v_arr, calc_x_a_v_arr = calc_handle(new_x_a_v_arr, new_x_arr[0], new_x_v_arr[0])
    # 计算y
    old_y_v_arr, old_y_a_v_arr = old_handle(y_arr)
    new_y_arr, new_y_v_arr, new_y_a_v_arr = handle1(y_arr, frac)
    calc_y_arr, calc_y_v_arr, calc_y_a_v_arr = calc_handle(new_y_a_v_arr, new_y_arr[0], new_y_v_arr[0])


    # XY对比
    o_data = {'x': x_arr, 'y': y_arr, 'l': 'Original'}
    l_data = {'x': new_x_arr, 'y': new_y_arr, 'l': 'Lowess'}
    c_data = {'x': calc_x_arr, 'y': calc_y_arr, 'l': 'Calc'}
    label = {'x': 'X Center', 'y': 'Y Center'}

    draw(o_data, l_data, c_data, label, 'XY_1 (' + str(frac) + ')', out_path)
    draw(None, l_data, c_data, label, 'XY_2 (' + str(frac) + ')', out_path)

    # X速度对比
    o_data = {'x': x_arr, 'y': old_x_v_arr, 'l': 'Original'}
    l_data = {'x': new_x_arr, 'y': new_x_v_arr, 'l': 'Lowess'}
    c_data = {'x': calc_x_arr, 'y': calc_x_v_arr, 'l': 'Calc'}
    label = {'x': 'X Center', 'y': 'X_V'}

    draw(o_data, l_data, c_data, label, 'X_V_1 (' + str(frac) + ')', out_path)
    draw(None, l_data, c_data, label, 'X_V_2 (' + str(frac) + ')', out_path)

    # X加速度对比
    o_data = {'x': x_arr, 'y': old_x_a_v_arr, 'l': 'Original'}
    l_data = {'x': new_x_arr, 'y': new_x_a_v_arr, 'l': 'Lowess'}
    c_data = {'x': calc_x_arr, 'y': calc_x_a_v_arr, 'l': 'Calc'}
    label = {'x': 'X Center', 'y': 'X_A_V'}

    draw(o_data, l_data, c_data, label, 'X_A_V_1 (' + str(frac) + ')', out_path)
    draw(None, l_data, c_data, label, 'X_A_V_2 (' + str(frac) + ')', out_path)

    # Y速度对比
    o_data = {'x': x_arr, 'y': old_y_v_arr, 'l': 'Original'}
    l_data = {'x': new_x_arr, 'y': new_y_v_arr, 'l': 'Lowess'}
    c_data = {'x': calc_x_arr, 'y': calc_y_v_arr, 'l': 'Calc'}
    label = {'x': 'X Center', 'y': 'Y_V'}

    draw(o_data, l_data, c_data, label, 'Y_V_1 (' + str(frac) + ')', out_path)
    draw(None, l_data, c_data, label, 'Y_V_2 (' + str(frac) + ')', out_path)

    # Y加速度对比
    o_data = {'x': x_arr, 'y': old_y_a_v_arr, 'l': 'Original'}
    l_data = {'x': new_x_arr, 'y': new_y_a_v_arr, 'l': 'Lowess'}
    c_data = {'x': calc_x_arr, 'y': calc_y_a_v_arr, 'l': 'Calc'}
    label = {'x': 'X Center', 'y': 'Y_A_V'}

    draw(o_data, l_data, c_data, label, 'Y_A_V_1 (' + str(frac) + ')', out_path)
    draw(None, l_data, c_data, label, 'Y_A_V_2 (' + str(frac) + ')', out_path)
