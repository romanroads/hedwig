...
def get_box(pose, frame_width, frame_height):
    """
    get expand bbox surrounding single agent's keypoints
    :param pose:
    :param frame_width:
    :param frame_height:
    :return:
...
def orb_matching(frame1, frame2, mask1, mask2, orb, flann_matcher):

    # find the keypoints and descriptors with ORB
    kp1, des1 = orb.detectAndCompute(frame1, mask=mask1)
    kp2, des2 = orb.detectAndCompute(frame2, mask=mask2)
...
def cal_grade(li, w):
    return sum(np.array(li) * np.array(w))
